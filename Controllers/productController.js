...
export async function createProduct(req, res) {
   if (!isAdmin(req)) {
    ...
    try {
        ...
        return res.status(500).json({ massage: "Failed to create product" })
    }
}
...
export async function getCategories(req, res) {
    try {
        const categories = await Product.distinct("category"); // products වල unique category names
        res.json(categories);
    } catch (error) {
...
export async function deleteProduct(req,res){
   if(!isAdmin(req)){
     res.status(403).json({massage: "Access dinied.Admin only"});
    return;
    }
